...
export class AuthService {
  ...
  async forceChangePassword(
    ...
    const hashedOld = await this.hashPassword(body.oldPassword);
    console.log(hashedOld, user.password);
  ...
  async changeForgottenPassword(dto: ChangeForgottenPasswordInputDto) {
    ...
    if (!user) {
      throw new SimpleNotFound("Żądanie zmiany hasła wygasło. Proszę spróbować ponownie.");
    }
  ...
  async validateForgotPasswordToken(token: string): Promise<boolean> {
    const user = await this.authRepository.getUserFromChangePasswordToken(token);
    if (!user) {
      return false;
    }
    return true;
