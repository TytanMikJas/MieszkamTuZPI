...
import { ANNOUNCEMENT_SORTING_OPTION_NEWEST_LABEL, DISTRICTS } from '@/strings';
...
>((set, get) => ({
  ...
  postAnnouncement: (
    ...
    formData.append('address', announcement.address);
  ...
  patchAnnouncement: (
    ...
    if (announcement.locationY)
      formData.append('locationY', announcement.locationY.toString());
    if (announcement.area) formData.append('area', announcement.area);
    if (announcement.address) formData.append('address', announcement.address);
