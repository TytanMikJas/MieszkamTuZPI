...
import IconButton from 'src/reusable-components/IconButton';
import { Preview, PreviewImage, PreviewFallback } from 'src/shadcn/preview';
import {
...
export default function CreateAnnouncementForm({ edit }: { edit?: boolean }) {
  ...
  useEffect(() => {
    if (singleAnnouncement && edit) {
      ...
      form.setValue('street', singleAnnouncement.address?.split(' ')[0]);
      const __n = singleAnnouncement.address.split(DOUBLE_WHITESPACE);
      let nrs: string[] = [];
      let _H = '';
      if (__n.length > 1) {
        nrs = __n[1].split('/');
      ...
      form.setValue('houseNumber', _H);
      form.setValue('apartmentNumber', _A);
  ...
  const onSubmit: SubmitHandler<AnnouncementFormData> = (data) => {
    ...
    const dto: AnnouncementInputDto = {
      ...
      address: `${data.street.trim()}${DOUBLE_WHITESPACE}${data.houseNumber.trim()}${data?.apartmentNumber != undefined && data?.apartmentNumber !== '' ? `/${data.apartmentNumber.trim()}` : ''}`,
    ...
    const files: File[] = [...extractUniqueFiles([...uploadedFiles, data.thumbnail[0]], [])];
  ...
  const onEdit: SubmitHandler<AnnouncementFormData> = (data) => {
    ...
    if (initialSingleAnnouncement.address === dto.address) {
      delete dto.address;
    }
    ...
    const files: File[] = [...extractUniqueFiles([...uploadedFiles, ...thumbnailfile], [...filenamesToIgnore, initialUploadedThumbnail[0].fileName])];
    const error = validateFiles(files, ANNOUNCEMENT_NAME);
